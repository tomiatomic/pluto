...
using DelimitedFiles, LsqFit, Plots, PlutoUI,  FFTW, LaTeXStrings, DSP, Interpolations, Statistics

# ╔═╡ f03eb307-16cc-428f-924e-e057181428d4
TableOfContents()

# ╔═╡ 5290a6ce-89ae-4ed9-b02f-f1e6033e07dd
md"""# Superconductor - Insulator - Superconductor tunnelling
check *e.g.*:\
...
*e.g.* "C:\Users\PC\OneDrive - UPJŠ\Dokumenty\data\SPECS\4H-NbSe2\4H-2.txt"
"""

# ╔═╡ 4bdb8190-d8d3-4246-9dc9-5e3c908f371b
@bind file FilePicker()

# ╔═╡ bb9a9121-c088-49ee-ab3b-80d47103adad
md"""Data filename: $(file["name"])"""

# ╔═╡ 76c6d89b-ea02-44b8-ad23-b3a5a08b85c8
...
# ╔═╡ 34df3f08-4ec1-44c8-a0a7-14954d522b15
begin
	plot(bias, cond, xlabel = "Bias voltage [mV]", ylabel = "dI/dV [a.u.]", title = "raw data", legend = false)
end
...
# ╔═╡ 0b4a1ce3-d204-4847-a047-6917ff50c269
# BCS density of states (with Dynes broadening gamma)
	  function bcs_dos(u, del, gam)
	      E_complex = u .+ im * gam
	      return real.((E_complex) ./ sqrt.(E_complex.^2 .- del^2))
	  end

# ╔═╡ 0b9ace77-a043-4a12-95c1-d56856202b02
...
# ╔═╡ bd72b9ca-4ab2-46ce-aafa-967c6dba5ec0
 # S-I-S convolution model (asymmetric junction: delta1 ≠ delta2)
 function sis(bias, delta1, delta2, T, gamma)
	      Emax = maximum(abs, bias) + 10 * max(delta1, delta2)
	      E = range(-Emax, stop=Emax, length=200_000)
	
	      N1 = model_dos(E, [delta1, gamma, T])
	      N2 = model_dos(E, [delta2, gamma, 0.00001])
	      
	      conv_raw = conv(N1, N2)
	      pad = div(length(conv_raw) - length(E), 2)
	      conv_signal = conv_raw[pad+1 : pad+length(E)]
	
	      itp = interpolate((E,), conv_signal, Gridded(Linear()))
	      interp_curve = itp.(bias)
	
	      tail_range = round(Int, length(bias) * 0.1)
	      tail_mean = mean(vcat(interp_curve[1:tail_range], interp_curve[end-tail_range+1:end]))
	      interp_curve ./= tail_mean
	
	      return interp_curve
end

# ╔═╡ 026b32a2-1809-413d-ad2c-3662fdd9c77b
# S-I-S convolution model (asymmetric junction: delta1 ≠ delta2)
 function sis2(bias, delta1, delta2, T, gamma)
	      Emax = maximum(abs, bias) + 10 * max(delta1, delta2)
	      E = range(-Emax, stop=Emax, length=200_000)
...
# ╔═╡ 84c98fd5-2be7-415b-aa0b-4e4ed6c46a7d
const kB = 8.617333262145e-2  # Boltzmann constant in meV/K

# ╔═╡ c99d9888-041f-475a-b03f-f897720195c6
# Fermi-Dirac distribution derivative at temperature T
	  fd_derivative(E, T) = -1 ./ (4 * kB * T * cosh.(E ./ (2 * kB * T)).^2)

# ╔═╡ 50665940-a8c0-420c-93c3-a6725bc67fa1
 # S-I-S convolution model (asymmetric junction: delta1 ≠ delta2)
	  function sis_conductance_asym(bias::Vector{Float64}, delta1::Float64, delta2::Float64, T::Float64, gamma::Float64=0.001)
	      Emax = maximum(abs, bias) + 10 * max(delta1, delta2)
	      E = range(-Emax, stop=Emax, length=200_000)
	
	      N1 = bcs_dos(E, delta1, gamma)
	      N2 = bcs_dos(E, delta2, gamma)
	      f_prime = fd_derivative.(E, T)
	
	      conv_raw = conv(N1 .* f_prime, N2)
	      pad = div(length(conv_raw) - length(E), 2)
	      conv_signal = conv_raw[pad+1 : pad+length(E)]
...
begin
	...
	del1_dynes = @bind del1_dynes Slider(0.00:0.01:2.00, 1.0, true)
	gam1_dynes = @bind gam1_dynes Slider(0.00:0.01:0.05, 0.011, true)
	del2_dynes = @bind del2_dynes Slider(0.00:0.01:2.00, 0.5, true)
	gam2_dynes = @bind gam2_dynes Slider(0.00:0.01:0.05, 0.011, true)
	md"Fitting parameters: \
	``T=`` $(t)K ``\pm`` $(terror)%\
	\
	``\Delta_1``: $(del1_dynes) meV \
	``\Gamma_1``: $(gam1_dynes) meV \
	``\Delta_2``: $(del2_dynes) meV \
	``\Gamma_2``: $(gam2_dynes) meV"
	
end

# ╔═╡ 12a0a02c-be7b-4018-9db4-00c05e3643d0
begin
	plot(bias, bcs_dos(bias, del2_dynes, gam2_dynes), alpha = 0.8, label = "Benedek")
	plot!(bias, dynes(bias, del2_dynes, gam2_dynes), ls = :dash, label = "Dynes")
end

# ╔═╡ 8c8a3cd1-7fc3-4700-9039-80e737d87b33
p1_dos=[del1_dynes, gam1_dynes, t]

# ╔═╡ d48b41de-2008-45ba-b6f1-07a6143cf82b
p2_dos=[del2_dynes, gam2_dynes, t]

# ╔═╡ 2c255601-3a0d-4e1a-9118-9f92c97dc4ce
begin
	plot(bias, model_dos(bias, p1_dos),
				label="Δ_1", 	
				ylabel = "Conductance [a.u.]", 
				xlabel = "Bias [meV]")
	plot!(bias, model_dos(bias, p2_dos),
				label="Δ_2", 	
				ylabel = "Conductance [a.u.]", 
				xlabel = "Bias [meV]")
end

# ╔═╡ e4871325-de32-4a55-92ce-d27a6f4b6be3
plot(sis(bias, del1_dynes, del2_dynes, t, gam1_dynes))

# ╔═╡ 336e2e76-2092-4156-8ce7-c61eca2c3243
plot(sis2(bias, del1_dynes, del2_dynes, t, gam1_dynes))

# ╔═╡ 736e2ed0-8a4f-47f9-b9e7-ec5285700b4f
plot(fd_derivative.(bias, t))

# ╔═╡ 5f5cefb0-c4b0-428b-8298-eacd5cb980d2
plot(bias, sis_conductance_asym(bias, del1_dynes, del2_dynes, t, gam1_dynes))

# ╔═╡ 0814c675-10c1-437d-991c-0afd183b4548
begin
	dos1 = bcs_dos(bias, del1_dynes, gam1_dynes)
	dos2 = bcs_dos(bias, del2_dynes, gam1_dynes)
	f_prime = fd_derivative.(bias, t)
	conv_raw = conv(dos1 .* f_prime, dos2)
	plot(1 .- conv_raw, xlims = (600, 1913-600))
end

# ╔═╡ 6dcfb729-cad7-4096-a616-6ef880201b87
begin
	dyn1 = dynes(bias, del1_dynes, gam1_dynes)
	dyn2 = dynes(bias, del2_dynes, gam1_dynes)
	conv_dyn = conv(dyn1 .* f_prime, dyn2)
	plot(conv_dyn)
end
...
manifest_format = "2.0"
project_hash = "da40e05347b0b50d035c3f1c767b5008cd54a2df"
