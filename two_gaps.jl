...
using PlutoUI, Plots, DelimitedFiles, FFTW, LsqFit
...
md"## Bardeen - Cooper - Schrieffer superconducting Density of States:
...
begin
	v = @bind v NumberField(0.00:50.00, default = 10.00)
	md"### Energy range ``\pm`` $(v) meV"
end

# ╔═╡ 4dd85b90-6953-49cb-bab0-033abe4079dd
begin
	del = @bind del NumberField(0.00:0.01:10.00, default = delcrit)
	md"### ``\Delta_0=`` $(del) meV"
end

# ╔═╡ a47f447f-346b-4bca-868d-f53a7d8248f8
u = range(start = -v, stop = v, length = 10001)
#u = bias # use voltages from experiment
...
begin
	plot(u, N, title ="$(dos) DOS at 0 K", lw = 2, legend = false,
			ylabel = "DOS[a.u.]", 
			xlabel = "E[meV]")
end
...
begin
	plot(u, NT, title ="$(dos) DOS at $(t) K", lw = 2, legend = false,
			ylabel = "DOS[a.u.]", 
			xlabel = "E[meV]",
			ylims = (0, maximum(NT) + 0.2))
end

# ╔═╡ 4daa429d-19c4-49ce-8d39-3da985d584c4
md"## Weighted mean of two gaps
[e.g. here](https://journals.aps.org/prb/abstract/10.1103/PhysRevB.76.212504), but also anisotropic papers..."

# ╔═╡ 85267a61-df6a-4057-89bc-df72ce151e02
md"### ``N(E) = w N_1(\Delta_1, \Gamma_1) + (1-w) N_2(\Delta_2, \Gamma_2)  ``"

# ╔═╡ 9beb3c98-e556-4474-9691-9eb6bc1a407a
begin
	gap1 = @bind gap1 Slider(0.00:0.01:2.00, 0.5*del, true)
	gap2 = @bind gap2 Slider(0.00:0.01:2.00, 1.5*del, true)
	gam1 = @bind gam1 Slider(0.01:0.001:2.2, gam, true)
	gam2 = @bind gam2 Slider(0.01:0.001:2.2, gam, true)
	md"Parameters [meV]: \
	``\Delta_1``: $(gap1) \
	``\Delta_2``: $(gap2) \
	``\Gamma_1``: $(gam1) \
...
# ╔═╡ ce8baafe-9e49-41e2-a612-9df0e2a5f6b7
begin
	w = @bind w Slider(0:0.01:1.0, default = .3, show_value=true)
	md"### ``weight_{\Delta_1}`` $(w)"
end

# ╔═╡ f6108a17-a63e-4b13-8778-9f7f24c1c3bc
#calculate DOS with 2 isotropic gaps using alpha (k), gap1 and gap2
twoiso = w*dos(u, gap1, gam1) + (1-w)*dos(u, gap2, gam2)

# ╔═╡ 32a4a2c9-d175-4cf5-8cae-a90e17ee98f8
begin
	plot(u, twoiso, title="Two isotropic gaps at 0K", lw = 2,
			label = false,
			ylabel = "DOS[a.u.]", 
			xlabel = "E[meV]",
			ylims = (0, :auto))
...
# ╔═╡ dbc6e68f-0aa8-4ac1-ae73-8da6af9ffd10
twoisoT = convol(u, twoiso, t)

# ╔═╡ 3128aadc-f0a0-4bd4-9f23-604a20e8b474
begin
	plot(u, twoisoT, title="Two isotropic gaps at $(t)K", lw = 2,
			label = false,
			ylabel = "DOS[a.u.]", 
			xlabel = "E[meV]",
			ylims = (0, :auto))
...
# ╔═╡ 8010a5cf-486c-41c4-9358-6480f34ce414
 function model_2iso(bias, p)
	 #calculate DOS with 2 isotropic gaps using alpha (k), gap1 and gap2
 	 zero = p[5]*dos(bias, p[1], p[3]) + (1-p[5])*dos(bias, p[2], p[4])
	 # temperature
...
# ╔═╡ 786543bf-83a3-44d6-b9bf-f4a41d0401c4
md"#### accuracy test"

# ╔═╡ 30828fa5-ab3e-4324-bfb1-2caced6fd7cc
begin
	first = [0.79, 1.02, 0.05, 0.05, 71.5, 0.44]
	second = [0.76, 0.97, 0.07, 0.07, 30, 0.55]
	plot(bias, model_2iso(bias, first), lw = 2, lc = 1,
				title = "Two isotropic gaps comparison",
				label="$(first)", 	
				ylabel = "Conductance [a.u.]", 
				xlabel = "Bias [meV]")
	plot!(bias, model_2iso(bias, second), lw = 2, lc = 0, ls = :dash,
				label="$(second)")
...
md"""## Dynes/BCS fit
Fitting parameters are: $\Delta_0, \Gamma, T$"""

# ╔═╡ dc9bc3ee-1b34-4e00-9dc7-7e0598a04ff0
function model_dos(bias, p)
	 #calculate DOS with either Dynes or BCS DOS ()
 	 zero = dos(bias, p[1], p[2])
	 # temperature
...
lo_dos = [0.0, 0.01, t - 0.1*t]

# ╔═╡ 143c6d1e-f2f9-4f52-b0db-e3aa92cbb5e6
up_dos = [2.0, 0.2, t + 0.1*t]
...
md"#### Fit result: ``\Delta_0 \approx`` $(round(fitdos.param[1], digits = 2)) meV, ``\Gamma \approx`` $(round(fitdos.param[2], digits = 2))  meV, ``T \approx`` $(round(fitdos.param[3], digits = 2)) K"
...
manifest_format = "2.0"
project_hash = "2d010df6b25d8394924209c708be8755eccb0c23"
