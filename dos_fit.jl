...
# ╔═╡ 1d225152-61c6-423e-a8d8-1b09da10d248
...
begin
	t = @bind t NumberField(0.01:0.01:10, default = 0.55)
...
### $\frac{2\Delta}{k_BT_c}=X \implies 2\Delta[meV] \approx \frac{X}{11.6}T_c[K]$"
...
# ╔═╡ f6b12354-d2c0-487b-a663-f583650198cf
md"# Fitting DOS models
Density of states models comparison:
...
md"#### Initial guess and boundaries"

# ╔═╡ b327b438-d53e-496e-b798-d3771a4e3fbc
begin
	del_dynes = @bind del_dynes Slider(0.00:0.01:10.00, delcrit, true)
	gam_dynes = @bind gam_dynes Slider(0.001:0.01:0.5, 0.011, true)
	md"Fitting parameters (besides temperature): \
	``\Delta``: $(del_dynes) meV \
	``\Gamma``: $(gam_dynes) meV"
end

# ╔═╡ 5613c56c-965a-4f79-bdfb-077fc1e616b6
...
begin
	plot(bias, cond, lw = 2, lc = "orange",
				...
				xlabel = "Bias [meV]")
	plot!(bias, model_dos(bias, p0_dos), label="initial guess", lw = 2, lc = 1)
end

# ╔═╡ 4a6a4ebb-f45f-4e65-b044-ca065d07e9da
fitdos = curve_fit(model_dos, bias, cond, p0_dos, lower = lo_dos, upper = up_dos)
...
begin
	plot(bias, cond, lw = 2, lc = "orange",
				...
				ylabel = "Conductance [a.u.]", 
				xlabel = "Bias [meV]")
	plot!(bias, model_dos(bias, fitdos.param), lw = 2, lc = 1,
				label="LsqFit")
end
...
# ╔═╡ 07dbf1b3-6061-4ccc-a847-db8ecfd52d00
md"## Maki pair breaking
[Assig et al., Rev. Sci. Instrum. '13] (https://pubs.aip.org/aip/rsi/article-abstract/84/3/033903/357814/A-10-mK-scanning-tunneling-microscope-operating-in?redirectedFrom=fulltext) \
[Maki et al., PoTP '64](https://academic.oup.com/ptp/article/31/6/945/1863147)
### ``N(E,\zeta)=\Re\left(\frac{u}{\sqrt{u^2-1}}\right); ~ u = \frac{E}{\Delta} + \zeta \frac{u}{\sqrt{1-u^2}}``"

# ╔═╡ 2efa8599-29aa-4a11-9c01-605b833f3257
begin
	function maki(E, delta, alfa)
	    # calculates the density of states from Maki formula
		ee = E ./ delta
	    eta = alfa / delta  # pair breaking parameter
	    b = @. ee^2 + (eta^2 - 1)
	    c = @. 108 * ee^2 * eta^2 + 2 * b^3 + sqrt(complex((108 * ee.^2 * eta^2 + 2 * b^3)^2 - 4 * b^6))
	    d = @. b / 3 + 2^(1/3) * b^2 / (3 * c^(1/3)) + c^(1/3) / (3 * 2^(1/3))
	    u = @. (1/2) * ( abs(ee) + sqrt(complex(1 - eta^2 + d)) + sqrt(complex(1 + ee^2 - eta^2 - d - (2 * abs(ee) * (1 + eta^2)) / sqrt(complex(1 - eta^2 + d)))) )
	    N = @. real( u / sqrt(complex(u^2 - 1)) )  # N is the DOS
	    return N
	end
	function model_maki(bias, p)
	 #calculate DOS at finite temperature with Maki equations
 	 zero = maki(bias, p[1], p[2])
	 # temperature
	 result = convol(bias, zero, p[3])
 end
end

# ╔═╡ f90a54a8-d8d5-44f0-bd26-b4bc2c55306b
md"#### Initial guess and boundaries"

# ╔═╡ e118f6e8-1b57-4ae1-afc9-532c0871be2a
begin
	del_maki = @bind del_maki Slider(0.00:0.01:10.00, delcrit, true)
	zeta = @bind zeta Slider(0.001:0.001:0.5, 0.01, true)
	md"Fitting parameters (besides temperature): \
	``\Delta``: $(del_maki) meV \
	``\zeta``: $(zeta) meV"
end

# ╔═╡ e3029b4b-a558-401b-846e-57af07c594b5
p0_maki=[del_maki, zeta, t]

# ╔═╡ fb390335-5ec6-4ab3-b130-ac3d0d169b94
lo_maki = [0.0, 0.0, t - t*terror/100]

# ╔═╡ 7bbc4067-e564-4f66-99d5-96446752ff9b
up_maki = [2.0, 1.0, t + t*terror/100]

# ╔═╡ b3f5066f-cefa-4ffa-b566-8219671c6a6c
begin
	plot(bias, cond, lw = 2, lc = "orange",
				title = "Maki DOS guess",
				label="experiment", 	
				ylabel = "Conductance [a.u.]", 
				xlabel = "Bias [meV]")
	plot!(bias, model_maki(bias, p0_maki), label="initial guess", lw = 2, lc = 1)
end

# ╔═╡ bc53ed0a-bacb-4c98-b152-351e0e354f76
fitmaki = curve_fit(model_maki, bias, cond, p0_maki, lower = lo_maki, upper = up_maki)

# ╔═╡ 5c4b183b-f8c1-4a8a-945e-e811b643ffa2
begin
	plot(bias, cond, lw = 2, lc = "orange",
				title = "Maki DOS fit",
				label="experiment", 	
				ylabel = "Conductance [a.u.]", 
				xlabel = "Bias [meV]")
	plot!(bias, model_dos(bias, fitmaki.param), lw = 2, lc = 1,
				label="LsqFit")
end

# ╔═╡ 5e422c82-319a-4805-9338-f6ed4621157f
md"### Fit result: ``\Delta \approx`` $(round(fitmaki.param[1], digits = 2)) meV, ``\zeta \approx`` $(round(fitmaki.param[2], digits = 2))  meV, ``T \approx`` $(round(fitmaki.param[3], digits = 2)) K"

# ╔═╡ eb312f43-6fc8-49f0-bc98-6ca08b17d0b4
md"""## McMillan proximity model
...
We iterate these equations N (*e.g.* N = 500) times to obtain initial conditions for the system of differential equations obtained by differentiating these expressions and solve it. [theses.fr](https://theses.fr/2004PA066555)
### $\Delta_i(E) = \frac{\Delta_i^0 + \Gamma_{ij}\Delta_j(E)/\sqrt{\Delta_j^2(E)-E^2}}{1+\Gamma_{ij}\sqrt{\Delta_j^2(E)-E^2}}$
"""

# ╔═╡ 170a442f-fc9a-4392-9727-ea08e5ef60b1
md"#### Density of states
#### ``N_i(E)=\Re\left(\frac{E}{\sqrt{E^2-\Delta_i^2(E)}}\right)``
#### ``N_{total} = w N_1(E) + (1-w) N_2(E)  ``"

# ╔═╡ 8349b1d2-8a37-41ca-bf4c-8a27d837caa7
begin
	function bcs(u, del)
	...
	function soldif(x, y, e, del1, del2, g1, g2)
		...
		prob = ODEProblem(deldif, u0, tspan, par) # choose ronan or deldif
...
# ╔═╡ ffe943a4-40e2-4f4b-9a2a-c07980710f44
md"""#### Initial guess and boundaries"""
...
begin
	...
	``\Gamma_1``: $(gam1_mcm) \
	``\Gamma_2``: $(gam2_mcm) \
	``weight_{\Delta_1}``: $(w_mcm)"
end

# ╔═╡ aecc0bf6-f538-41cc-b269-c9512532c384
p0_mcm=[del1_mcm, del2_mcm, gam1_mcm, gam2_mcm, w_mcm, t]

# ╔═╡ a9cd569f-9fb3-4490-962c-b49da5298497
lo_mcm = [0.0, 0.0, 0.01, 0.01, 0.0, t - t*terror/100]

# ╔═╡ cc68e910-d0bf-49e3-a8a3-85ae7db22ff6
up_mcm = [2.0, 2.0, 5.0, 5.0, 1.0, t + t*terror/100]

# ╔═╡ 4082651e-40cd-4dd4-8c7e-0c444c4886f6
begin
	plot(bias, cond, lw = 2, lc = "orange",
				title = "McMillan guess",
				label="experiment", 	
				ylabel = "Conductance [a.u.]", 
				xlabel = "Bias [meV]")
...
# ╔═╡ 05e7cb93-285f-4a50-a785-80b08742ec1b
fit_mcm = curve_fit(mcmillan, bias, cond, p0_mcm, lower = lo_mcm, upper = up_mcm)

# ╔═╡ 4961e1c5-cfb0-4281-83ce-d8304a8d1cd4
begin
	plot(bias, cond, lw = 2, lc = "orange",
				title = "McMillan fit",
				label="experiment", 	
...
# ╔═╡ 9601696c-7025-438c-b9d7-420f4ea4d1ca
...
# ╔═╡ c3045dac-d825-426e-86e3-89dfc55349c7
md"## Anisotropic gap with rotational symmetry
Links to papers:
[STM](https://www.nature.com/articles/srep09408),
[Heat capacity](https://iopscience.iop.org/article/10.1088/1674-1056/17/6/048)
### ``\Delta(\theta)=\Delta_0(1+\alpha*cos(6\theta))``"

# ╔═╡ 4f91b1cb-dbe5-408b-be16-49a715953610
begin
	function aniso(theta, del0, alpha)
		del0 .* (1 .+ alpha .* cos.(6 .*theta))
	end
	function model_ani(bias, p)
		# generate vector of theta angles
		theta = range(start = 0, stop = pi/3, length = 100)
		# gap size at theta angles
		dani = aniso(theta, p[1], p[3])
		#calculate gap size for each theta angle
		Ntheta = zeros(length(bias),length(dani))
			for i in eachindex(dani)
				Ntheta[:,i] = dynes(bias, dani[i], p[2]) 
			end
		#summarize along rows, normalize to sum of first row and change to vector
		Naniso = vec(sum(Ntheta, dims = 2)./sum(Ntheta[1,:]))
		#apply Fermi thermal smearing
		NanisoT = convol(bias, Naniso, p[4])
	end
end

# ╔═╡ 9149a63e-d4d9-4701-b503-a1f833bb27fc
md"""#### Initial guess and boundaries
Number of $\theta$ steps in "model\_ani" must be defined manually!!! (Not a fitting parameter, integer.)"""

# ╔═╡ 50af773b-52ea-4014-8179-b07bcd7236e4
begin
	del_ani = @bind del_ani Slider(0.00:0.01:10.00, delcrit, true)
	gam_ani = @bind gam_ani Slider(0.001:0.01:0.5, 0.011, true)
	al = @bind al Slider(0.00:0.01:1.00, 0.2, true)
...
# ╔═╡ 3a5e8ad9-eb56-4654-82ed-b623fa6c9f51
p0_ani = [del_ani, gam_ani, al, t]

# ╔═╡ 93ac8d15-06cc-4280-979c-ccbd07942c50
lo_ani = [0.0, 0.01, 0.0, t - t*terror/100]

# ╔═╡ 8cc28333-5291-4026-9038-bd932f912f31
up_ani = [2.0, 1.0, 1.0, t + t*terror/100]

# ╔═╡ 74b7eac9-4b17-418f-8de5-3487e60b127a
begin
	plot(bias, cond, lw = 2, lc = "orange",
				title = "Anisotropic gap guess",
				label="experiment", 	
				ylabel = "Conductance [a.u.]", 
				xlabel = "Bias [meV]")
	plot!(bias, model_ani(bias, p0_ani), label="initial guess", lw = 2, lc = 1)
end

# ╔═╡ 68bf09f7-a288-41a5-b9dc-bed930c697de
fitani = curve_fit(model_ani, bias, cond, p0_ani, lower = lo_ani, upper = up_ani)

# ╔═╡ ab2f9f79-ed15-4c68-8985-53e6be93853b
begin
	plot(bias, cond, lw = 2, lc = "orange",
				title = "Anisotropic gap fit",
				label="experiment", 	
				ylabel = "Conductance [a.u.]", 
				xlabel = "Bias [meV]")
	plot!(bias, model_ani(bias, fitani.param), lw = 2, lc = 1,
				label="LsqFit")
end

# ╔═╡ 7891ca5a-34e7-4678-bdc4-ba6fdfc991fd
md"### Fit result: ``\Delta_0 \approx`` $(round(fitani.param[1], digits = 2)) meV, ``\Gamma \approx`` $(round(fitani.param[2], digits = 2))  meV, ``\alpha \approx`` $(round(fitani.param[3], digits = 2)), ``T \approx`` $(round(fitani.param[4], digits = 2)) K"

# ╔═╡ 46fd38de-86a3-4cdc-8465-cbf8cc06c81c
md"## Linear combination of two gaps
[e.g. here](https://journals.aps.org/prb/abstract/10.1103/PhysRevB.76.212504), but also anisotropic papers...
### ``N(E) = w N_1(\Delta_1, \Gamma_1) + (1-w) N_2(\Delta_2, \Gamma_2)``"

# ╔═╡ 8010a5cf-486c-41c4-9358-6480f34ce414
 function model_2iso(bias, p)
	 #calculate DOS with 2 isotropic gaps using alpha (k), gap1 and gap2
 	 zero = p[5]*dynes(bias, p[1], p[3]) + (1-p[5])*dynes(bias, p[2], p[4])
	 # temperature
	 result = convol(bias, zero, p[6])
 end

# ╔═╡ bb83e300-9ae2-40ea-84a8-f1fce1fbfa92
md"""#### Initial guess and boundaries"""

# ╔═╡ 9beb3c98-e556-4474-9691-9eb6bc1a407a
begin
	gap1 = @bind gap1 Slider(0.00:0.01:2.00, 0.5*delcrit, true)
	gap2 = @bind gap2 Slider(0.00:0.01:2.00, 1.5*delcrit, true)
	gam1 = @bind gam1 Slider(0.01:0.001:2.2, 0.011, true)
	gam2 = @bind gam2 Slider(0.01:0.001:2.2, 0.011, true)
	w = @bind w Slider(0:0.01:1.0, default = .3, show_value=true)
	md"Fitting parameters (besides temperature): \
	``\Delta_1``: $(gap1) meV \
	``\Delta_2``: $(gap2) meV\
	``\Gamma_1``: $(gam1) meV\
...
# ╔═╡ 9b61a527-43d5-4356-8b3c-079fdabae74d
p0_2iso=[gap1, gap2, gam1, gam2, w, t]

# ╔═╡ d29f19c6-fcb8-4ce1-9949-32fc0de97d80
lo_2iso = [0.0, 0.0, 0.01, 0.01, 0.0, t - t*terror/100]

# ╔═╡ 592989f5-db77-48ab-9dd5-89bbdd7db80e
up_2iso = [2.0, 2.0, 2.0, 2.0, 1.0, t + t*terror/100]

# ╔═╡ 8be424c2-0d7b-48eb-8638-d1fd51c1382d
begin
	plot(bias, cond, lw = 2, lc = "orange",
				title = "Two isotropic gaps guess",
				label="experiment", 	
				ylabel = "Conductance [a.u.]", 
				xlabel = "Bias [meV]")
	plot!(bias, model_2iso(bias, p0_2iso), label="initial guess", lw = 2, lc = 1)
end

# ╔═╡ 76096b74-451a-4ca8-97d4-3a03a234b36d
fit2iso = curve_fit(model_2iso, bias, cond, p0_2iso, lower = lo_2iso, upper = up_2iso)

# ╔═╡ f7bab4dd-0ea4-4cac-a557-eefba2e452e3
begin
	plot(bias, cond, title ="Experimental data and fitted models", lw = 2, lc = "black", label = "experiment",
			ylabel = "Differential onductance [a.u.]", 
			xlabel = "Bias [meV]")
	plot!(bias, model_dos(bias, fitdos.param), label = "Dynes")
	plot!(bias, model_dos(bias, fitmaki.param), label = "Maki")
	plot!(bias, mcmillan(bias, fit_mcm.param), label = "McMillan")
	plot!(bias, model_ani(bias, fitani.param), label = "anisotropic")
	plot!(bias, model_2iso(bias, fit2iso.param), label = "2 gaps", ls = :dash)
end

# ╔═╡ 417cb787-5261-4c29-b9ea-789612f69f67
begin
	plot(bias, cond, lw = 2, lc = "orange",
				title = "Two isotropic gaps fit",
				label="experiment", 	
				ylabel = "Conductance [a.u.]", 
				xlabel = "Bias [meV]")
...
# ╔═╡ 42d96cb1-990e-41c2-8449-e1067ab7f701
md"### Fit result: ``\Delta_1 \approx`` $(round(fit2iso.param[1], digits = 2)) meV,  ``\Delta_2 \approx`` $(round(fit2iso.param[2], digits = 2)) meV, ``\Gamma_1 \approx`` $(round(fit2iso.param[3], digits = 2))  meV, ``\Gamma_2 \approx`` $(round(fit2iso.param[4], digits = 2))  meV, ``weight_{\Delta_1} \approx`` $(round(fit2iso.param[5], digits = 2)), ``T \approx`` $(round(fit2iso.param[6], digits = 2)) K"
