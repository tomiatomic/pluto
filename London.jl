...
using Plots, PlutoUI, NLsolve; 	plotly()

# ╔═╡ bdbef7b1-6f1f-4133-aa85-a80577fef5e4
TableOfContents()

# ╔═╡ e10c11b1-cd37-4c51-a0a2-2a3689a85e1a
md"# London penetration depth *vs.* temperature for different values of ``\frac{2\Delta}{k_bT_c}``
eq. 1.34, pg 21 (pdf 23), [Local magnetometry measurements of selected
superconductors, Zuzana Medvecka](https://theses.hal.science/tel-01738228)"

# ╔═╡ ff94e41b-2d0c-4515-8486-0d9c0805b97a
md"""
## Approximate calculation of ``\Delta(T)`` using *tanh*
...
begin
	critemp = @bind critemp NumberField(0.10:0.01:20.00, default = 5.15)
	md"### ``T_c=`` $(critemp)K"
end

# ╔═╡ 2ade0aed-e8d8-481a-b4fc-bb9ce7da7eea
begin
	x = @bind x Slider(0.01:0.01:10.00, 0.45, true)
	md"### ``\frac{2\Delta}{k_bT_c}=`` $(x)"
end
...
function lam_t(d0, reddel, t)
	kb=8.617343*10^(-5)
	@. reddel*tanh(d0*reddel/(2*kb*t))
end

# ╔═╡ 4be897a9-bc11-4178-9513-9f32e552d3f8
begin
	london = lam_t(del_0, del_red, temp)
	plot(temp, london, xlabel = "T [K]", lw = 2, legend = false)
end
...
manifest_format = "2.0"
project_hash = "88cd838ea5965de0dfd931d187d2cd9f109e6237"
