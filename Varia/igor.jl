...
md"""# Pauli paramagnetic limit:
(From a preliminary intro on 22.7.2022 to our [PRB '23 paper](https://journals.aps.org/prb/abstract/10.1103/PhysRevB.108.L220501).) \
...
* ``\mu_B \approx 5.8 \times 10^{-5}~~eV.T^{-1}`` is the [Bohr magneton](https://en.wikipedia.org/wiki/Bohr_magneton)
* ``k_B \approx 8.62 \times 10^{-5}~~eV.K^{-1}`` is the [Boltzmann constant](https://en.wikipedia.org/wiki/Boltzmann_constant)
#### ``B_t \approx \frac{3.53}{2}\frac{k_B}{\mu_B}T_c \approx2.6 × T_c ~~[T/K]``.
"""

# ╔═╡ f28d8687-89b6-46f3-8471-1f5f63b59183
begin
	kb = 8.617333262e−5
	mb = 5.7883818060e−5
...
# ╔═╡ ddff5432-0661-4964-acff-75243791e00d
md"""
![hcp](https://github.com/tomiatomic/pics/blob/main/hcp.png?raw=true)
"""
...
# ╔═╡ ad80da16-2be7-4c3b-887f-ee81af931a35
begin
	zbi = @bind zbi Slider(0.00:1.0:100.00, 75.0, true)
	tbi_min = @bind tbi_min Slider(0.0:0.01:10.0, 0.0, true)
	tbi_max = @bind tbi_max Slider(10.1:0.1:100.00, 100.0, true)
md"### Plotting the function:
``\zeta`` = $(zbi) meV \
``t_{min}`` = $(tbi_min) meV \
``t_{max}`` = $(tbi_max) meV"
end

# ╔═╡ 33ef2056-b807-42e1-af4d-8faed754982e
begin
	plotly()
	xbi = range(tbi_min, tbi_max, length = 1000)
	ybi = bilayer(zbi, xbi, 1)
	plot(xbi,ybi, xlabel = "t[meV]", ylabel = "B<sub>c</sub>/B<sub>P</sub>", title = "ζ = $(zbi)", legend = false)
end
...
# ╔═╡ 1b2b334a-dbe9-4cfb-b8da-08d8d6206506
begin
	zas = @bind zas Slider(1.00:1.0:100.00, 75.0, true)
	tas_min = @bind tas_min Slider(0.00:0.01:0.99, 0.0, true)
	tas_max = @bind tas_max Slider(1.00:1.0:1000.00, 200.0, true)
md"### Plotting the function:
``\zeta`` = $(zas) meV \
``T_0^{min}`` = $(tas_min) meV \
``T_0^{max}`` = $(tas_max) meV"
...
# ╔═╡ 8a2e6a08-b227-4c5e-8928-8337e7576492
begin
	plotly()
	xas = range(tas_min, tas_max, length = 1000)
	yas = asym(zas, xas)
	plot(xas,yas, xlabel = "T<sub>0</sub>[meV]", ylabel = "B<sub>c</sub>[T]", title = "ζ = $(zas)", legend = false)
end

# ╔═╡ 597d4d8f-cd0d-458b-969d-346b96a11a8f
begin
	gr()
	plot(xas,yas, legend = :inside, xlabel = L"T_0[meV]", ylabel = L"B_c[T]", label = "ζ = $(zas)")
end
