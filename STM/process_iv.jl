...
using DelimitedFiles, LsqFit, Plots, PlutoUI,  FFTW, Interpolations, FiniteDifferences, Loess
...
begin
	left_peak = @bind left_peak NumberField(-100.0:100.0, default = 0)
	right_peak = @bind right_peak NumberField(-100.0:100.0, default = 0)
...
begin
	...
	# remove background
	#linear fit of 1/4 outer range
	quart = Int(round(length(cut_bias)/4)) #interval used as normal conductance
	linfit = linear_fit([cut_cond[1:quart]; cut_cond[end-quart+1:end]])
...
begin
	t = @bind t NumberField(0.01:0.01:10, default = 0.5)
