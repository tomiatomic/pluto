...
# ╔═╡ b80f580e-75d2-11ef-26aa-55702afdd5b9
using Symbolics, Plots, PlutoUI

# ╔═╡ 29892e39-a580-4853-b1e5-7d4f98e13039
@variables x a b

# ╔═╡ 9fe52c47-0e73-4453-a5a8-654999c80170
f = a * x^2 + b * x + 1

# ╔═╡ e7bba121-ae9d-4558-baff-837e4809d2b3
f_num = eval(build_function(f, x, a, b))

# ╔═╡ 0d18dbed-964a-4f36-b655-c3738cbd743a
begin
	reset
	a_val = @bind a_val Slider(0.0:0.01:4.0, 1.15, true)
	b_val = @bind b_val Slider(0.0:0.01:40.0, 1.0, true)
	md"Parameters: a = $(a_val), b = $(b_val)"
end

# ╔═╡ ae6bced4-3244-47a4-92e5-6b7775e8eb98
begin
	x_vals = range(-10, 10, length=100)
	y_vals = f_num.(x, a_val, b_val)
	plot(x_vals, y_vals, label="f(x) = $(a_val)x^2 + $(b_val)x + 1", xlabel="x", ylabel="f(x)")
end
...
manifest_format = "2.0"
project_hash = "5b7f668619a44c2b171cdce9f8321532e3bf743f"
