...
begin
	plot(u, NT, title ="$(dos) DOS at $(t) K", lw = 2,
			ylabel = "DOS[a.u.]", 
			xlabel = "E[meV]",
			legend = false,
...
begin
	al = @bind al Slider(0.00:0.01:1.00, default = 0.2, show_value=true)
	md"### $\alpha$ $(al)"
end
...
# generate vector of theta angles
	polar = range(start = 0, stop = 2*pi, length = 360)
# gap size at theta angles
	
	plot(polar, aniso(polar, del, al), proj = :polar, m = 2, title = "Anisotropic gap",label = "Δ [meV]")
end

# ╔═╡ cad0d678-4428-46a3-9697-3f3014112cb3
begin
	st = @bind st NumberField(10:10:10000, default = 100)
	md"### ``\theta~steps`` $(st)"
end
...
begin
# generate vector of theta angles
	theta = range(start = 0, stop = pi/3, length = Int(st))
# gap size at theta angles
	dani = aniso(theta, del, al)
	plot(theta, dani, lw = 2,
			title = "θ range with $(st) steps used for the model",
			label = false,
			ylabel = "Δ[meV]", 
			xlabel = "θ[rad]")
end
...
#calculate DOS for each gap size (one column is one DOS)
	begin
		Ntheta = zeros(length(u),length(dani))
		for i = 1:length(dani)
			Ntheta[:,i] = dos(u, dani[i], gam) 
		end
		Ntheta
	end
...
begin
	plot(u, Ntheta, title="DOS at each θ", lw = 2,
			label = false,
			ylabel = "DOS[a.u.]", 
			xlabel = "E[meV]")
end
...
NanisoT = convol(u, Naniso, t)

# ╔═╡ 5201abfe-0500-4217-a5f8-68473c688cc3
begin
	plot(u, NanisoT, lw = 2,
			title = "Anisotropic gap at $(t) K",
			legend = false, 
			ylabel = "DOS[a.u.]", 
			xlabel = "E[meV]")
end
...
md"# Fitting models to experiment"
