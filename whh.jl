...
using Plots, PlutoUI, NLsolve, DelimitedFiles, LsqFit
...
function whh_sparse(F, x, par)
	# x = (4/(pi^2)) * [Hc2(T)/(-dHc2/dT)T=Tc] 
    v = -1000:1000
	t = par[1] # t = T/Tc
	a = par[2] # a = alpha = Maki parameter = sqrt(2) * H(orb)/H(paramag)
	l = par[3] # l = lambda = spin-orbit constant
	term1 = -log(1/t)
	term2 = @. 1/abs(2*v+1) - 
			(abs(2*v+1) + (x/t) + (a*x/t)^2/(abs(2*v+1)+(x+l)/t) )^(-1)
	F[1] =  term1 + sum(term2) # F =  term1 + (term2...) not working! 
end

# ╔═╡ 4fdeab85-ebd9-4844-9f3a-e5af76841a7b
function fit_whh(temp, p)
	alpha = p[1].*ones(length(temp))
	lambda = p[2].*ones(length(temp))
	pars = hcat(temp/p[3], alpha, lambda) # [temp'; alpha'; lambda']'
	initial_guess = [1.0]
...
begin
	plotly()
	temp = range(0.001,1,1001) # zero crashes
	alpha = al.*ones(length(temp))
	lambda = lam.*ones(length(temp))
	pars = hcat(temp, alpha, lambda) # [temp'; alpha'; lambda']'
	initial_guess = [1.0]
	sol = zeros(length(temp))
	for i in 1:length(temp)
		parameters = pars[i,:]
	...
	plot(temp,sol, legend = false, title = "Eq. 28 solution")
end		

# ╔═╡ 1074043c-b690-4c55-bd3a-91aa85514b2a
begin
	tempguess = range(0.1, tc, 1001)
	scatter(tii, cii, label = "experiment", xlabel = "T [K]", ylabel = "Critical magnetic field [T]", title = "guess")
	plot!(tempguess, fit_whh(tempguess, [al, lam, tc, db]), label = "initial guess")
end
...
begin
	@bind z CheckBox()
	md"Shoud I fit the data? It takes a while... $(@bind z CheckBox())"
end
...
begin
	if z == true
        fitpar = [al, lam, tc, db]
		fitwhh = curve_fit(fit_whh, tii, cii, fitpar)
		tempfit = range(0.01, fitwhh.param[3], 1001)
		scatter(tii, cii, label = "experiment", xlabel = "T [K]", ylabel = "Critical magnetic field [T]", title = "fit")
		plot!(tempfit, fit_whh(tempfit, fitwhh.param), label = "fit")
		else
...
# same as whh_sparse but less summands for faster fit
function whh_dense(F, x, par)
	# x = (4/(pi^2)) * [Hc2(T)/(-dHc2/dT)T=Tc] 
    v = -100000:100000
	t = par[1] # t = T/Tc
	a = par[2] # a = alpha = Maki parameter = sqrt(2) * H(orb)/H(paramag)
...
begin
	ini = [1.0]
	paras = [0.7, 0.1, 0.01] #reduced temperature, alpha, lambda
	test = nlsolve((F, x) -> whh_dense(F, x, paras), ini)
	println("Test: ", test)
end

# ╔═╡ a2efb1be-35b8-4669-86a0-1bb11c42043b
function plot_whh(temp, p)
	alpha = p[1].*ones(length(temp))
	lambda = p[2].*ones(length(temp))
	pars = hcat(temp/p[3], alpha, lambda) # [temp'; alpha'; lambda']'
	initial_guess = [1.0]
	...
	for i in 1:length(temp)
		parameters = pars[i,:]
		result = nlsolve((F, x) -> whh_dense(F, x, parameters), initial_guess)
		sol[i] = result.zero[1]
	end
	result = sol.*p[4]
end		
...
begin
	if z == true
	templot = range(0.01, fitwhh.param[3], 1001)
	scatter(tii, cii, label = "experiment", xlabel = "T [K]", ylabel = "Critical magnetic field [T]", title = "whh_dense with fit parameters")
	plot!(templot, plot_whh(templot, fitwhh.param), label = "fit")
		else
		println("Waiting for checkbox...")
	end
end
...
begin
	if z == true
       DownloadButton([collect(templot)'; plot_whh(templot, fitwhh.param)']', "whh_fit.txt") # download generated curve
	else
		println("Waiting for checkbox...")
	end
end
