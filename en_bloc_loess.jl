...
begin
	...
	del_dynes = @bind del_dynes Slider(0.00:0.01:10.00, 0.2, true)
...
begin
	...
	plot!(cut_bias, lorentz, label="Lorentzian")
...
begin
	if fit1 == true
		fitdos = curve_fit(model_dos, cut_bias, norm_cond./lorentz, p0_dos, lower = lo_dos, upper = up_dos)
		fit_first = model_dos(cut_bias, fitdos.param)
		plot(cut_bias, norm_cond./lorentz,
				title = "Dynes DOS fit",
				label="experiment", 	
				ylabel = "Normalized dI/dV [a.u.]", 
				xlabel = "Bias [meV]")
		plot!(cut_bias, fit_first,label="LsqFit")
		else
...
begin
	if proc == true
		...
		# surface plot
		surface(cut_bias, temps, red_mat,
...
# ╔═╡ 01f505a8-e604-4170-8315-0fec0c7d77e7
begin
	if remove_out == true
...
begin
	...
	# highlight first curve
		plot!(cut_bias,                     # x-axis
		    fill(temps_rem[1], length(cut_bias)),  # y-axis (constant temperature)
...
# ╔═╡ 41731ae7-e103-4885-946d-10cf5ee770db
begin
	if remove_out == true
	# plot Dynes fit results
	scatter(temps_rem, deltas_rem, label = "Δ from Dynes fit ", xlabel = "T [K]", ylabel = "Δ [meV]")
	...
	# plot delta(T) fit
	temp_range = 0.0:0.001:maximum(temps_rem)
	plot!(temp_range, fitanh(temp_range, [tc_estim, del_estim]), label = "Δ(T) estimate")
	else
		println("Waiting for checkbox...")
	end
end
...
md"""#### My estimate: ``T_c = `` $tc_estim K, ``\Delta_0 =`` $del_estim meV, ``\frac{2\Delta}{k_BT_c} \approx`` $(round(2*11.6*del_estim/tc_estim, digits = 2))
"""
