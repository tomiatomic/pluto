...
md"""# Process __*I(V) vs. B*__ en bloc"""
...
md""" Select file type: $(@bind controller Select([1 => "WSxM", 2 => "Nanonis", 3 => "raw data"]))"""
...
begin
	...
	fields_raw = Float64[] #vector of temperatures in K
	...
	files = glob("*Avg.iv.cur", folder)
		...
		for file in files
			...
			# Pali 11-12-05
			m = match(r"(\d+)mT_(\d+)_Current", basename(file)) 
			...
			push!(fields_raw, parse(Float64, m.captures[1]) / 1000.0)  # Convert mK to K
	...
	order = sortperm(fields_raw)
	fields = fields_raw[order]
	nums = nums_raw[order]
...
plot(U_matrix[1], I_matrix[:], xlabel = "Bias voltage [mV]", ylabel = "Tunneling current [pA]", title = "$(length(fields)) curves", label = string.(fields'))

# ╔═╡ 3fdc633d-f0e6-47cb-b35c-3337710fadbd
md"""## Process zero field spectrum"""

# ╔═╡ 5d830960-1b18-4343-a52f-8e5eec9233b7
begin
	cur = I_matrix[1]
	raw_bias = U_matrix[1]
	plot(raw_bias, cur, xlabel = "Bias voltage [mV]", ylabel = "Tunneling current [pA]", title = "$(fields[1]) T", legend = false)
end
...
# ╔═╡ 03b42aca-589f-46d2-a798-609909ca174d
md"#### Use Lorentzian to normalize? $(@bind lorentz_on CheckBox())"

# ╔═╡ be60854d-af84-4c11-bd62-2f2c5d96b009
begin
	par_a = @bind par_a Slider(1.00:0.01:2.00, 1.0, true)
	par_b = @bind par_b Slider(0.0:0.1:10.00, 2.0, true)
	par_c = @bind par_c Slider(1.1:0.01:10.00, 2.0, true)

	md"``y = a - \frac{1}{(bx)^2+c^2}`` \
	a = $(par_a), b = $(par_b), c = $(par_c)"
end

# ╔═╡ 2c255601-3a0d-4e1a-9118-9f92c97dc4ce
begin
	# Lorenzian
	lorentz = @. par_a - float(lorentz_on)/((par_b*cut_bias)^2+par_c^2)

	plot(cut_bias, lorentz,
	title = "",
	label="Lorentzian",
	ylabel = "Normalized dI/dV [a.u.]", 
	xlabel = "Bias [meV]")
	plot!(cut_bias, norm_cond./lorentz, label="experiment/Lorentz")
end

# ╔═╡ 9e13f528-8c03-4af2-8255-bdb73396f195
md"""## Process spectra en bloc"""

# ╔═╡ a95c4858-673f-4aa2-b800-e57f3dffc0b3
@warn "⚠️ Dividing the conductance by inverted Lorentzian!!! (unless unchecked)"
...
begin
	if proc == true
		...
		plot(cut_bias, norm_b[:], label = string.(fields'), xlabel = "Bias voltage [mV]", ylabel = "Normalized dI/dV [a.u.]", title = "Processed spectra", legend=:bottomleft)
...
begin
	if proc == true
		...
		red_mat = reduce(vcat, [v' for v in vec(norm_b)])
		
		# surface plot
		surface(cut_bias, fields, red_mat,
		...
		    fill(fields[1], length(cut_bias)),  # y-axis (constant temperature)
		    red_mat[1, :],                 # z-axis (first column of red_mat)
...
begin
	if proc == true
	md"Plot curve nr. $(@bind cur_nr NumberField(1:length(fields), default = length(fields)))"
	else
		println("Waiting for checkbox...")
	end
end
...
begin
	if proc == true
		plot(cut_bias, norm_b[cur_nr], legend = false, xlabel = "Bias voltage [mV]", ylabel = "Normalized dI/dV [a.u.]", title = "B = $(fields[cur_nr]) B")
...
begin
	if proc == true
		if div_cur == true
			sel_cur = norm_b[cur_nr]
			# Divide each of the first 49 vectors by sel_cur
		norm_b_div = [v ./ sel_cur for v in norm_b[1:end]]
		else
			norm_b_div = norm_b
		end
		
		plot(cut_bias, norm_b_div[:], label = string.(fields'), xlabel = "Bias voltage [mV]", ylabel = "Normalized dI/dV [a.u.]", title = "Processed spectra", legend=:bottomleft)
...
begin
	if proc == true
		#vec(mat) flattens the 1×59 matrix into a vector of vectors, reduce(vcat, ...) stacks them into a 2D matrix.
		red_mat_div = reduce(vcat, [v' for v in vec(norm_b_div)])
		
		# surface plot
		surface(cut_bias, fields, red_mat_div,
		    xlabel = "Bias voltage [mV]",
		    ylabel = "Mag. field [T]",
		    zlabel = "Normalized dI/dV [a.u.]",
		    color = :turbo,
...
# ╔═╡ bd252725-f468-4558-ac54-9052d6b595e8
md"""#### Remove spectra"""

# ╔═╡ 1b277430-6374-40dd-8398-975a7981f958
begin
	if proc == true
	md"Plot curve nr. $(@bind rem_nr NumberField(1:length(fields), default = length(fields)))"
	else
		println("Waiting for checkbox...")
	end
end

# ╔═╡ b22a90da-af27-4278-84cf-9ebf267d1552
begin
	if proc == true
		plot(cut_bias, norm_b_div[rem_nr], legend = false, xlabel = "Bias voltage [mV]", ylabel = "Normalized dI/dV [a.u.]", title = "B = $(fields[rem_nr]) B")
...
# ╔═╡ c1ccdaca-b9b6-47bc-9b97-872c800bb2bc
md"""input indexes to remove in cell below..."""

# ╔═╡ 553755a1-cd72-468e-bb2d-8e16c3fd7a0f
begin
	if proc == true
		# Indices to remove
		last = length(fields)
		remove_indices = [40:last...]
		#remove_indices = [0]
		
		# Keep only the elements not in remove_indices
		fields_rem = fields[setdiff(1:length(fields), remove_indices)]
		red_mat_rem = red_mat_div[setdiff(1:size(red_mat_div, 1), remove_indices),:]
	
		# plot remaining spectra
		surface(cut_bias, fields_rem, red_mat_rem, xlabel = "Bias voltage [mV]", ylabel = "Mag. field [T]", zlabel="Normalized dI/dV [a.u.]", color=:turbo, colorbar=false, zlims = (0.0, :auto))
	
		# highlight first curve
			plot!(cut_bias,                     # x-axis
			    fill(fields_rem[1], length(cut_bias)),  # y-axis (constant temperature)
			    red_mat_rem[1, :],                 # z-axis (first column of red_mat)
				line = (:black, 5, :solid, 1.0),  # color, width, style(:solid, :dash, :dot, :dashdot), alpha
			    label = "$(fields_rem[1])T")
	else
	println("Waiting for checkbox...")
	end
end

# ╔═╡ 6f564bae-5696-4ec5-8de1-23c514341ac5
begin
	if proc == true
...
# ╔═╡ 9276f796-8f25-49cd-8e78-66fac39b77bf
md"""## Download processed data
filename prefix: $(@bind save_prefix TextField(70, default="test"))\
Original folder: $(folder)
"""

# ╔═╡ 949c01e4-1cd3-4e23-b261-d7fed42077eb
begin
	if proc == true
	### Save processed data to a temporary file
	savename = string(save_prefix, "_mag_dep.jld2")
	@save savename fields_rem cut_bias red_mat_rem
